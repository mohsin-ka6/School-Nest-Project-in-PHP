...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'delete') {
    ...
    try {
        ...
        $stmt_img = $db->prepare("SELECT image_path FROM news_and_events WHERE id = ?");
        ...
        $stmt = $db->prepare("DELETE FROM news_and_events WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['action'])) {
    ...
    if (empty($errors)) {
        try {
            if ($item_id > 0) { // Update
                $stmt = $db->prepare("UPDATE news_and_events SET title=?, content=?, image_path=?, event_date=?, type=?, status=? WHERE id=?");
                $stmt->execute([$title, $content, $image_db_path, $event_date, $type, $status, $item_id]);
                $_SESSION['success_message'] = "Item updated successfully!";
            } else { // Insert
                $stmt = $db->prepare("INSERT INTO news_and_events (title, content, image_path, event_date, type, status) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([$title, $content, $image_db_path, $event_date, $type, $status]);
...
if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['id'])) {
    $stmt = $db->prepare("SELECT * FROM news_and_events WHERE id = ?");
    $stmt->execute([(int)$_GET['id']]);
    $edit_item = $stmt->fetch();
}

// Fetch all items
$items = $db->query("SELECT * FROM news_and_events ORDER BY created_at DESC")->fetchAll();
